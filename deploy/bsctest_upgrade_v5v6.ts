...
async function main() {
  ...
  const proxy = await upgrades.upgradeProxy(core, BSCTestV6, { 
    kind: 'uups', useDeployedImplementation: true, call: 'initializeV6'
   });
  await proxy.deployTransaction.wait();
  const newImpl = await upgrades.erc1967.getImplementationAddress(core);
  console.log(`BSCTest upgrade V5 to V6: network=${network.name}, core=${core}, \
